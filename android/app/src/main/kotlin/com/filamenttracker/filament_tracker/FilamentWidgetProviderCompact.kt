...
class FilamentWidgetProviderCompact : AppWidgetProvider() {
    override fun onUpdate(
        ...
        for (appWidgetId in appWidgetIds) {
            val views = RemoteViews(context.packageName, R.layout.filament_widget_compact)
            ...
            try {
                ...
                views.setTextViewText(R.id.widget_spool_count, "$spoolCount Spulen")

                val lowStockInt = lowStock.toIntOrNull() ?: 0
                if (lowStockInt > 0) {
                    views.setTextViewText(R.id.widget_low_stock, "⚠️ $lowStockInt")
                    views.setTextColor(R.id.widget_low_stock, android.graphics.Color.parseColor("#FF5252"))
                } else {
                    views.setTextViewText(R.id.widget_low_stock, "✓ OK")
